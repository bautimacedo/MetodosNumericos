...
double func (double x){
	...
	return 35.4*exp(pow(x,2))-26.9;
}
	
	
	int main(int argc, char *argv[]) {
		double m[4][2]= {{0.5,-0.716},{0.8,-0.103},{1.3,3.41},{2.0,52.7}};
		int filas=4; 
		//fileReader(m,&filas);
		int seleccion;
		printf("\ncantidad de filas %d\n",filas);
		...
		void lagrange (double m[FILAS][2], int fila){
			...
			for (int i = 0; i < fila; i++) {
				...
				for (int j = 0; j < fila; j++) {
					if (j != i) { //Verificamos que j e i sean distintos porque sino estariamos dividiendo por 0
			...
			void polinomica (double m[FILAS][2], int fila){
				...
				for (int i = 0; i < fila; i++) {
					for (int j = 0; j < fila +1 ; j++) {
						a[i][j] = pow(m[i][0], j); //almacena coeficientes de las potencias de "x"
					}
					b[i] = m[i][1]; //almacena valores "y"
				}
				printf("\nMatriz a\n"); //Matriz con valores de los coeficientes de potencia
				for (int i = 0; i < fila; i++) {
					for (int j = 0; j < fila; j++) {
						printf("%lf",a[i][j]);
						printf("\t");
				...
				printf("\nMatriz b\n"); //Matriz con los valores "y" correspondientes a los puntos de entrada
				for (int i = 0; i < fila; i++) {
					printf("%lf",b[i]);
					printf("\t");
				}
